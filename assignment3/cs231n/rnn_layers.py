...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ## It's interesthing that the bias term does not have to be the right size
    cache = {}
    cache['prev_h'] = prev_h
    cache['Wh'] = Wh
    cache['x'] = x
    cache['Wx'] = Wx
    cache['tanhvalue'] = next_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    x = cache['x']
    Wx = cache['Wx']
    tanhvalue = cache['tanhvalue']
    dtanh = (1 - np.square(tanhvalue)) * dnext_h
    db = np.sum(dtanh,axis=0)
    dWx = np.dot(x.T,dtanh)   #(D,H) = (D,N)(N,H)
    dx = np.dot(dtanh, Wx.T)    #N,D = (N,H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##############################################################################
    pass
    N, T, D = np.shape(x) # number of sequence
    H = len(b)
    h = np.zeros((N,T,H))
    cache = {}
    for seq in range(T): # operating N sample in parallel and T state sequentially
        x_comp = x[:,seq,:] #(N,D)
        
        if(seq == 0):
            next_h,cache[seq] = rnn_step_forward(x_comp, h0, Wx, Wh, b)     
        else:
            next_h,cache[seq] = rnn_step_forward(x_comp, next_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    N, T, H = np.shape(dh)
    #print(cache)
    N,D = np.shape(cache[0]['x'])
    
    dprev_h_seq = np.zeros((N,H))
    dx = np.zeros((N,T,D))
    dh0 = np.zeros((N,H))
    dWx = np.zeros((D,H))
    dWh = np.zeros((H,H))
    db = np.zeros((H))
    for seq in reversed(range(T)):
        dh_seq = dh[:,seq,:] # N,H
        if(seq == T):
            dx[:,seq,:], dprev_h_seq, dWx_seq, dWh_seq, db_seq = rnn_step_backward(dh_seq,cache[seq])
        else:
            dx[:,seq,:], dprev_h_seq, dWx_seq, dWh_seq, db_seq = rnn_step_backward(dh_seq+dprev_h_seq,cache[seq])
        dWx += dWx_seq
        dWh += dWh_seq
        db += db_seq
    dh0 = dprev_h_seq 
